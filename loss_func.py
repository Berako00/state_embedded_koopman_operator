...
def loss_2(xuk, Num_meas, model):
    total_g_loss = torch.tensor(0.0, device=xuk[:, 0, :].device)
    for m in range(0,len(xuk[0, :, 0])):
        pred = model.u_Decoder(model.u_Encoder(xuk[:, m, :]))
        total_g_loss += F.mse_loss(pred, xuk[:, m, Num_meas:], reduction='mean')
    L_gx = total_g_loss / len(xuk[0, :, 0])
...
def loss_3(xuk, Num_meas, model):
    pred_3 = model.x_Koopman_op(model.x_Encoder(xuk[:, 0, :Num_meas])) + model.u_Koopman_op(model.u_Encoder(xuk[:, 0, :]))
